...
function () {
  ...
  _createClass(Transaction, [{
    ...
    value: function _fetchSignData() {
      ...
      instructions.forEach(function (instruction) {
        var instructionLayout = struct([u8('controllerIdIndex'), blob(instruction.keyIndicesCount.length, 'keyIndicesCount'), seq(u8('keyIndex'), instruction.keyIndices.length, 'keyIndices'), blob(instruction.dataLength.length, 'dataLength'), seq(u8('userdatum'), instruction.data.length, 'data')]);
        var length = instructionLayout.encode(instruction, instructionBuffer, instructionBufferLength);
        instructionBufferLength += length;
      });
      instructionBuffer = instructionBuffer.slice(0, instructionBufferLength);
      var signDataLayout = struct([blob(1, 'numRequiredSignatures'), blob(keyCount.length, 'keyCount'), seq(pubKey('key'), keys.length, 'keys'), pubKey('recentBlockhash'), blob(controllerIdCount.length, 'controllerIdCount'), seq(pubKey('controllerId'), controllerIds.length, 'controllerIds')]);
      var transaction = {
...
var FetchClusterNodes = jsonRpcResult(struct$1.list([struct$1({
  id: 'string',
  gossip: 'string',
...
function () {
  ...
  _createClass(SystemController, null, [{
    ...
    value: function createNewAccount(from, newAccount, dif, space, controllerId) {
      var dataLayout = struct([u32('instruction'), ns64('dif'), ns64('space'), pubKey('controllerId')]);
...
function serializeCond(condition) {
  switch (condition.type) {
    ...
    case 'signature':
      {
        var dataLayout = struct([u32('condition'), pubKey('from')]);
...
export { BpfControllerLoader, BudgetController, BusAccount, Connection, ControllerLoader, NativeControllerLoader, PubKey, SystemController, Token, TokenCount, Transaction, TxOperation, sendAndConfmOriginalTxn, sendAndConfmTxn, testnetChannelEndpoint };
