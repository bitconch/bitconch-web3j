...
down)
  (
    set -x
    if [[ -n "$(docker ps --filter "name=^bus-localnet$" -q)" ]]; then
      docker stop --time 0 bus-localnet
    fi
  )
  ;;
logs)
  ...
  while $follow; do
    if [[ -n $(docker ps -q -f name=bus-localnet) ]]; then
      (
        set -x
        docker logs bus-localnet -f
      ) || true
    fi
    sleep 1
  done
...
deploy)
  ...
  if docker exec bus-localnet test -f /usr/bin/"$basename"; then
  ...
  (
    set -x
    docker cp "$program" bus-localnet:/usr/bin/
  )
  docker exec bus-localnet ls -l /usr/bin/"$basename"
