...
if [[ ! -f llvm-native-$machine-$version.md ]]; then
  (
    filename=bitconch-llvm-$machine.tar.bz2
    ...
    base=https://github.com/bitconch/llvm-builder/releases
...
if [[ ! -f rust-bpf-$machine-$version.md ]]; then
  (
    filename=bitconch-rust-bpf-$machine.tar.bz2
    ...
    base=https://github.com/bitconch/rust-bpf-builder/releases
    download $base/download/$version/$filename $filename giga
    tar -jxf $filename
    rm -rf $filename
    popd
...
if [[ ! -f rust-bpf-sysroot-$version.md ]]; then
  (
    ...
    rm -rf xargo
    cmd="git clone --recursive --single-branch --branch $version https://github.com/bitconch/rust-bpf-sysroot.git"
    $cmd
